...
export function SavedItemList({ items, type, onDelete, onArchive, onShare }: SavedItemListProps) {
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="font-semibold text-foreground mb-2">
            {searchQuery ? "No results found" : `No ${typeLabels[type].toLowerCase()} yet`}
          </h3>
          <p className="text-sm text-muted-foreground">
            {searchQuery
              ? "Try a different search term"
              : `Long-press chat messages to save them as ${typeLabels[type].toLowerCase()}`}
          </p>
        </div>
      ) : (
